...
def add(request):
    if request.method == "POST":
        try:
            ...
            required_fields = ['stransmission', 'sfuel', 'cname', 'cprice', 'mileage', 'seats', 'luggage', 'desc']
            for field in required_fields:
                if not request.POST.get(field):
                    msg = f"{field.title()} is required!"
            ...
            car = Car.objects.create(
                ...
                cprice=int(request.POST['cprice']),
            ...
            print(f"Successfully added car: {car.cname}")
            ...
            msg = "Please enter valid numbers for price, mileage, seats and luggage."
            return render(request, 'add.html', {'msg': msg})
        except Exception as e:
            print("Error adding car:", e)
            msg = "An error occurred while adding the car. Please try again."
            return render(request, 'add.html', {'msg': msg})
    else:
...
def view(request):
    try:
        ...
        print(f"Found {cars.count()} cars for lessor: {lessor.email}")
        
        return render(request, 'view.html', {'car': cars})
...
def update(request, pk):
    ...
    if request.method == 'POST':
        ...
        car.mileage = request.POST.get('milegae')
